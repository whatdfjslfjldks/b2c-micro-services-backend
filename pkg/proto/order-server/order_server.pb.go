// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.1
// 	protoc        v5.29.2
// source: order_server.proto

package order_server_proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type GetOrderDetailRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	OrderId       string                 `protobuf:"bytes,1,opt,name=order_id,json=orderId,proto3" json:"order_id,omitempty"`             // 订单号
	AccessToken   string                 `protobuf:"bytes,2,opt,name=access_token,json=accessToken,proto3" json:"access_token,omitempty"` // 检查身份，是否是买家
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetOrderDetailRequest) Reset() {
	*x = GetOrderDetailRequest{}
	mi := &file_order_server_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetOrderDetailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetOrderDetailRequest) ProtoMessage() {}

func (x *GetOrderDetailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_order_server_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetOrderDetailRequest.ProtoReflect.Descriptor instead.
func (*GetOrderDetailRequest) Descriptor() ([]byte, []int) {
	return file_order_server_proto_rawDescGZIP(), []int{0}
}

func (x *GetOrderDetailRequest) GetOrderId() string {
	if x != nil {
		return x.OrderId
	}
	return ""
}

func (x *GetOrderDetailRequest) GetAccessToken() string {
	if x != nil {
		return x.AccessToken
	}
	return ""
}

type GetOrderDetailResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Code          int32                  `protobuf:"varint,1,opt,name=code,proto3" json:"code,omitempty"`
	StatusCode    string                 `protobuf:"bytes,2,opt,name=status_code,json=statusCode,proto3" json:"status_code,omitempty"`
	Msg           string                 `protobuf:"bytes,3,opt,name=msg,proto3" json:"msg,omitempty"`
	OrderId       string                 `protobuf:"bytes,4,opt,name=order_id,json=orderId,proto3" json:"order_id,omitempty"` // 订单号
	OrderDate     string                 `protobuf:"bytes,5,opt,name=order_date,json=orderDate,proto3" json:"order_date,omitempty"`
	OrderStatus   int32                  `protobuf:"varint,6,opt,name=order_status,json=orderStatus,proto3" json:"order_status,omitempty"`
	PaymentMethod int32                  `protobuf:"varint,7,opt,name=payment_method,json=paymentMethod,proto3" json:"payment_method,omitempty"` // 支付方式
	PaymentStatus int32                  `protobuf:"varint,8,opt,name=payment_status,json=paymentStatus,proto3" json:"payment_status,omitempty"` // 支付状态
	PaymentPrice  float64                `protobuf:"fixed64,9,opt,name=payment_price,json=paymentPrice,proto3" json:"payment_price,omitempty"`   // 支付金额
	Name          string                 `protobuf:"bytes,10,opt,name=name,proto3" json:"name,omitempty"`                                        // 买家姓名
	Phone         string                 `protobuf:"bytes,11,opt,name=phone,proto3" json:"phone,omitempty"`
	Address       string                 `protobuf:"bytes,12,opt,name=address,proto3" json:"address,omitempty"`                                          // 地址
	Detail        string                 `protobuf:"bytes,13,opt,name=detail,proto3" json:"detail,omitempty"`                                            // 具体地址
	Note          string                 `protobuf:"bytes,14,opt,name=note,proto3" json:"note,omitempty"`                                                // 留言
	ProductId     []int32                `protobuf:"varint,15,rep,packed,name=product_id,json=productId,proto3" json:"product_id,omitempty"`             // 商品id数组
	TypeName      []string               `protobuf:"bytes,16,rep,name=type_name,json=typeName,proto3" json:"type_name,omitempty"`                        // 商品分类数组
	ProductAmount []int32                `protobuf:"varint,17,rep,packed,name=product_amount,json=productAmount,proto3" json:"product_amount,omitempty"` // 商品数量数组
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetOrderDetailResponse) Reset() {
	*x = GetOrderDetailResponse{}
	mi := &file_order_server_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetOrderDetailResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetOrderDetailResponse) ProtoMessage() {}

func (x *GetOrderDetailResponse) ProtoReflect() protoreflect.Message {
	mi := &file_order_server_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetOrderDetailResponse.ProtoReflect.Descriptor instead.
func (*GetOrderDetailResponse) Descriptor() ([]byte, []int) {
	return file_order_server_proto_rawDescGZIP(), []int{1}
}

func (x *GetOrderDetailResponse) GetCode() int32 {
	if x != nil {
		return x.Code
	}
	return 0
}

func (x *GetOrderDetailResponse) GetStatusCode() string {
	if x != nil {
		return x.StatusCode
	}
	return ""
}

func (x *GetOrderDetailResponse) GetMsg() string {
	if x != nil {
		return x.Msg
	}
	return ""
}

func (x *GetOrderDetailResponse) GetOrderId() string {
	if x != nil {
		return x.OrderId
	}
	return ""
}

func (x *GetOrderDetailResponse) GetOrderDate() string {
	if x != nil {
		return x.OrderDate
	}
	return ""
}

func (x *GetOrderDetailResponse) GetOrderStatus() int32 {
	if x != nil {
		return x.OrderStatus
	}
	return 0
}

func (x *GetOrderDetailResponse) GetPaymentMethod() int32 {
	if x != nil {
		return x.PaymentMethod
	}
	return 0
}

func (x *GetOrderDetailResponse) GetPaymentStatus() int32 {
	if x != nil {
		return x.PaymentStatus
	}
	return 0
}

func (x *GetOrderDetailResponse) GetPaymentPrice() float64 {
	if x != nil {
		return x.PaymentPrice
	}
	return 0
}

func (x *GetOrderDetailResponse) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *GetOrderDetailResponse) GetPhone() string {
	if x != nil {
		return x.Phone
	}
	return ""
}

func (x *GetOrderDetailResponse) GetAddress() string {
	if x != nil {
		return x.Address
	}
	return ""
}

func (x *GetOrderDetailResponse) GetDetail() string {
	if x != nil {
		return x.Detail
	}
	return ""
}

func (x *GetOrderDetailResponse) GetNote() string {
	if x != nil {
		return x.Note
	}
	return ""
}

func (x *GetOrderDetailResponse) GetProductId() []int32 {
	if x != nil {
		return x.ProductId
	}
	return nil
}

func (x *GetOrderDetailResponse) GetTypeName() []string {
	if x != nil {
		return x.TypeName
	}
	return nil
}

func (x *GetOrderDetailResponse) GetProductAmount() []int32 {
	if x != nil {
		return x.ProductAmount
	}
	return nil
}

type TestPaySuccessRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	OrderId       string                 `protobuf:"bytes,1,opt,name=order_id,json=orderId,proto3" json:"order_id,omitempty"` // 订单号
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TestPaySuccessRequest) Reset() {
	*x = TestPaySuccessRequest{}
	mi := &file_order_server_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TestPaySuccessRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TestPaySuccessRequest) ProtoMessage() {}

func (x *TestPaySuccessRequest) ProtoReflect() protoreflect.Message {
	mi := &file_order_server_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TestPaySuccessRequest.ProtoReflect.Descriptor instead.
func (*TestPaySuccessRequest) Descriptor() ([]byte, []int) {
	return file_order_server_proto_rawDescGZIP(), []int{2}
}

func (x *TestPaySuccessRequest) GetOrderId() string {
	if x != nil {
		return x.OrderId
	}
	return ""
}

type TestPaySuccessResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Code          int32                  `protobuf:"varint,1,opt,name=code,proto3" json:"code,omitempty"`
	StatusCode    string                 `protobuf:"bytes,2,opt,name=status_code,json=statusCode,proto3" json:"status_code,omitempty"`
	Msg           string                 `protobuf:"bytes,3,opt,name=msg,proto3" json:"msg,omitempty"`
	ReturnUrl     string                 `protobuf:"bytes,4,opt,name=return_url,json=returnUrl,proto3" json:"return_url,omitempty"` // 成功后回调地址，也就是下一个界面
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TestPaySuccessResponse) Reset() {
	*x = TestPaySuccessResponse{}
	mi := &file_order_server_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TestPaySuccessResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TestPaySuccessResponse) ProtoMessage() {}

func (x *TestPaySuccessResponse) ProtoReflect() protoreflect.Message {
	mi := &file_order_server_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TestPaySuccessResponse.ProtoReflect.Descriptor instead.
func (*TestPaySuccessResponse) Descriptor() ([]byte, []int) {
	return file_order_server_proto_rawDescGZIP(), []int{3}
}

func (x *TestPaySuccessResponse) GetCode() int32 {
	if x != nil {
		return x.Code
	}
	return 0
}

func (x *TestPaySuccessResponse) GetStatusCode() string {
	if x != nil {
		return x.StatusCode
	}
	return ""
}

func (x *TestPaySuccessResponse) GetMsg() string {
	if x != nil {
		return x.Msg
	}
	return ""
}

func (x *TestPaySuccessResponse) GetReturnUrl() string {
	if x != nil {
		return x.ReturnUrl
	}
	return ""
}

type GetAliPayQRCodeRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	OrderId       string                 `protobuf:"bytes,1,opt,name=order_id,json=orderId,proto3" json:"order_id,omitempty"` // 订单号
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAliPayQRCodeRequest) Reset() {
	*x = GetAliPayQRCodeRequest{}
	mi := &file_order_server_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAliPayQRCodeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAliPayQRCodeRequest) ProtoMessage() {}

func (x *GetAliPayQRCodeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_order_server_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAliPayQRCodeRequest.ProtoReflect.Descriptor instead.
func (*GetAliPayQRCodeRequest) Descriptor() ([]byte, []int) {
	return file_order_server_proto_rawDescGZIP(), []int{4}
}

func (x *GetAliPayQRCodeRequest) GetOrderId() string {
	if x != nil {
		return x.OrderId
	}
	return ""
}

type GetAliPayQRCodeResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Code          int32                  `protobuf:"varint,1,opt,name=code,proto3" json:"code,omitempty"`
	StatusCode    string                 `protobuf:"bytes,2,opt,name=status_code,json=statusCode,proto3" json:"status_code,omitempty"`
	Msg           string                 `protobuf:"bytes,3,opt,name=msg,proto3" json:"msg,omitempty"`
	CodeUrl       string                 `protobuf:"bytes,4,opt,name=code_url,json=codeUrl,proto3" json:"code_url,omitempty"` // aliPay的二维码地址
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAliPayQRCodeResponse) Reset() {
	*x = GetAliPayQRCodeResponse{}
	mi := &file_order_server_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAliPayQRCodeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAliPayQRCodeResponse) ProtoMessage() {}

func (x *GetAliPayQRCodeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_order_server_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAliPayQRCodeResponse.ProtoReflect.Descriptor instead.
func (*GetAliPayQRCodeResponse) Descriptor() ([]byte, []int) {
	return file_order_server_proto_rawDescGZIP(), []int{5}
}

func (x *GetAliPayQRCodeResponse) GetCode() int32 {
	if x != nil {
		return x.Code
	}
	return 0
}

func (x *GetAliPayQRCodeResponse) GetStatusCode() string {
	if x != nil {
		return x.StatusCode
	}
	return ""
}

func (x *GetAliPayQRCodeResponse) GetMsg() string {
	if x != nil {
		return x.Msg
	}
	return ""
}

func (x *GetAliPayQRCodeResponse) GetCodeUrl() string {
	if x != nil {
		return x.CodeUrl
	}
	return ""
}

type CreateOrderRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AccessToken   string                 `protobuf:"bytes,1,opt,name=access_token,json=accessToken,proto3" json:"access_token,omitempty"`               // token验明身份
	Address       string                 `protobuf:"bytes,2,opt,name=address,proto3" json:"address,omitempty"`                                          // 地址
	Detail        string                 `protobuf:"bytes,3,opt,name=detail,proto3" json:"detail,omitempty"`                                            // 详细地址
	Name          string                 `protobuf:"bytes,4,opt,name=name,proto3" json:"name,omitempty"`                                                // 买家姓名
	Phone         string                 `protobuf:"bytes,5,opt,name=phone,proto3" json:"phone,omitempty"`                                              // 买家手机号
	Note          string                 `protobuf:"bytes,6,opt,name=note,proto3" json:"note,omitempty"`                                                // 订单备注
	ProductId     []int32                `protobuf:"varint,7,rep,packed,name=product_id,json=productId,proto3" json:"product_id,omitempty"`             // 商品id数组
	TypeName      []string               `protobuf:"bytes,8,rep,name=type_name,json=typeName,proto3" json:"type_name,omitempty"`                        // 商品分类数组
	ProductAmount []int32                `protobuf:"varint,9,rep,packed,name=product_amount,json=productAmount,proto3" json:"product_amount,omitempty"` // 商品数量数组
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateOrderRequest) Reset() {
	*x = CreateOrderRequest{}
	mi := &file_order_server_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateOrderRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateOrderRequest) ProtoMessage() {}

func (x *CreateOrderRequest) ProtoReflect() protoreflect.Message {
	mi := &file_order_server_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateOrderRequest.ProtoReflect.Descriptor instead.
func (*CreateOrderRequest) Descriptor() ([]byte, []int) {
	return file_order_server_proto_rawDescGZIP(), []int{6}
}

func (x *CreateOrderRequest) GetAccessToken() string {
	if x != nil {
		return x.AccessToken
	}
	return ""
}

func (x *CreateOrderRequest) GetAddress() string {
	if x != nil {
		return x.Address
	}
	return ""
}

func (x *CreateOrderRequest) GetDetail() string {
	if x != nil {
		return x.Detail
	}
	return ""
}

func (x *CreateOrderRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateOrderRequest) GetPhone() string {
	if x != nil {
		return x.Phone
	}
	return ""
}

func (x *CreateOrderRequest) GetNote() string {
	if x != nil {
		return x.Note
	}
	return ""
}

func (x *CreateOrderRequest) GetProductId() []int32 {
	if x != nil {
		return x.ProductId
	}
	return nil
}

func (x *CreateOrderRequest) GetTypeName() []string {
	if x != nil {
		return x.TypeName
	}
	return nil
}

func (x *CreateOrderRequest) GetProductAmount() []int32 {
	if x != nil {
		return x.ProductAmount
	}
	return nil
}

type CreateOrderResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Code          int32                  `protobuf:"varint,1,opt,name=code,proto3" json:"code,omitempty"`
	StatusCode    string                 `protobuf:"bytes,2,opt,name=status_code,json=statusCode,proto3" json:"status_code,omitempty"`
	Msg           string                 `protobuf:"bytes,3,opt,name=msg,proto3" json:"msg,omitempty"`
	OrderId       string                 `protobuf:"bytes,4,opt,name=order_id,json=orderId,proto3" json:"order_id,omitempty"` // 只返回一个订单号，其他信息通过订单号获取
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateOrderResponse) Reset() {
	*x = CreateOrderResponse{}
	mi := &file_order_server_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateOrderResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateOrderResponse) ProtoMessage() {}

func (x *CreateOrderResponse) ProtoReflect() protoreflect.Message {
	mi := &file_order_server_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateOrderResponse.ProtoReflect.Descriptor instead.
func (*CreateOrderResponse) Descriptor() ([]byte, []int) {
	return file_order_server_proto_rawDescGZIP(), []int{7}
}

func (x *CreateOrderResponse) GetCode() int32 {
	if x != nil {
		return x.Code
	}
	return 0
}

func (x *CreateOrderResponse) GetStatusCode() string {
	if x != nil {
		return x.StatusCode
	}
	return ""
}

func (x *CreateOrderResponse) GetMsg() string {
	if x != nil {
		return x.Msg
	}
	return ""
}

func (x *CreateOrderResponse) GetOrderId() string {
	if x != nil {
		return x.OrderId
	}
	return ""
}

var File_order_server_proto protoreflect.FileDescriptor

var file_order_server_proto_rawDesc = []byte{
	,
}

var (
	file_order_server_proto_rawDescOnce sync.Once
	file_order_server_proto_rawDescData = file_order_server_proto_rawDesc
)

func file_order_server_proto_rawDescGZIP() []byte {
	file_order_server_proto_rawDescOnce.Do(func() {
		file_order_server_proto_rawDescData = protoimpl.X.CompressGZIP(file_order_server_proto_rawDescData)
	})
	return file_order_server_proto_rawDescData
}

var file_order_server_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_order_server_proto_goTypes = []any{
	(*GetOrderDetailRequest)(nil),   // 0: proto.GetOrderDetailRequest
	(*GetOrderDetailResponse)(nil),  // 1: proto.GetOrderDetailResponse
	(*TestPaySuccessRequest)(nil),   // 2: proto.TestPaySuccessRequest
	(*TestPaySuccessResponse)(nil),  // 3: proto.TestPaySuccessResponse
	(*GetAliPayQRCodeRequest)(nil),  // 4: proto.GetAliPayQRCodeRequest
	(*GetAliPayQRCodeResponse)(nil), // 5: proto.GetAliPayQRCodeResponse
	(*CreateOrderRequest)(nil),      // 6: proto.CreateOrderRequest
	(*CreateOrderResponse)(nil),     // 7: proto.CreateOrderResponse
}
var file_order_server_proto_depIdxs = []int32{
	6, // 0: proto.OrderService.CreateOrder:input_type -> proto.CreateOrderRequest
	4, // 1: proto.OrderService.GetAliPayQRCode:input_type -> proto.GetAliPayQRCodeRequest
	2, // 2: proto.OrderService.TestPaySuccess:input_type -> proto.TestPaySuccessRequest
	0, // 3: proto.OrderService.GetOrderDetail:input_type -> proto.GetOrderDetailRequest
	7, // 4: proto.OrderService.CreateOrder:output_type -> proto.CreateOrderResponse
	5, // 5: proto.OrderService.GetAliPayQRCode:output_type -> proto.GetAliPayQRCodeResponse
	3, // 6: proto.OrderService.TestPaySuccess:output_type -> proto.TestPaySuccessResponse
	1, // 7: proto.OrderService.GetOrderDetail:output_type -> proto.GetOrderDetailResponse
	4, // [4:8] is the sub-list for method output_type
	0, // [0:4] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_order_server_proto_init() }
func file_order_server_proto_init() {
	if File_order_server_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_order_server_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_order_server_proto_goTypes,
		DependencyIndexes: file_order_server_proto_depIdxs,
		MessageInfos:      file_order_server_proto_msgTypes,
	}.Build()
	File_order_server_proto = out.File
	file_order_server_proto_rawDesc = nil
	file_order_server_proto_goTypes = nil
	file_order_server_proto_depIdxs = nil
}
